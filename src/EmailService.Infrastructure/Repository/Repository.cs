...
public class Repository<TEntity> : IRepository<TEntity> where TEntity : Identity
{
    ...
    public IQueryable<TEntity> GetQurable() => _dbSet;
    ...
    public async Task<IEnumerable<TEntity>> GetPagedAsync(
        ...
        bool disableTracking = true)
    {
         var query = _dbSet.OrderBy(item => item.Id).Skip((pageNumber - 1) * pageSize).Take(pageSize);
    ...
    {
        IQueryable<TEntity> predicateResult = _dbSet;
        predicateResult.Where(predicate);

        if (include is not null) predicateResult = include(predicateResult);
       if (enableTracking) predicateResult = predicateResult.AsNoTracking();
